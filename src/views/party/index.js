...
import {
  ...
  ContentWrap,
  priceToString,
} from "../../components/main/bottomCard";
...
const Party = () => {
  ...
  //store
  const { platformCategoryList: categoryList } = useSelector(
    (state) => state.main.platform
  );
  const { reportPopupStatus } = useSelector((state) => state.party.popup);
  ...
  useEffect(async () => {
    ...
    setContentHeight(
      window.innerHeight - contentDivRef.current.getBoundingClientRect().top
    );
  ...
  const getPartyList = async () => {
    ...
    //벨리데이션
    if (partyListData.statusCode != 200) {
      return;
    }
    ...
    setMatchingCount(
      partyListData.result.filter((data) => data.roomStatus === "MATCHING")
        .length
    );
  ...
  const handleClickEnroll = () => {
    alert(
      "9월 6일에 모두 서비스가 종료될 예정이므로 파티 생성이 불가능합니다. 그동안 모두를 이용해주셔서 진심으로 감사드립니다. 🙇🏻‍♂️🙇🏻‍♀️"
    );
    return;
    ...
    <>
      ...
      >
        ...
        <div style={{ zIndex: "10" }}>
          ...
          <PartyIconWrap
            ...
            style={{ right: "3.375rem" }}
          >
            <img
              src={PartyTutorialIcon}
              style={{ width: "1.2188rem", height: "1.4375rem" }}
            />
          </PartyIconWrap>
          <PartyIconWrap
            onClick={() => {
          ...
          >
            <img
              src={MyPartyIcon}
              style={{ width: "1.5rem", height: "1.4375rem" }}
            />
          </PartyIconWrap>
          <div
            className="spoqaBold"
          ...
          >
            <span style={{ color: "#ffffff" }}>{matchingCount}</span> 개의
            파티가
            <br />
            파티원을 찾고 있어요!
          </div>
          <CategoryWrap className="spoqaBold">
            ...
            <CategoryItem
              onClick={() => onClickCategory(0, "전체")}
              isSelected={seletedCategory === 0}
            >
              전체
            </CategoryItem>
            {categoryList.map((value) => {
              return (
                <CategoryItem
                  onClick={() => {
                    onClickCategory(value.idx, value.name);
                  }}
                  isSelected={seletedCategory === value.idx}
                  key={value.idx}
                >
                  {value.name}
                </CategoryItem>
            ...
            {partyList.length === 0 ? (
              <div className="spoqaBold" style={{ textAlign: "center" }}>
                <div style={{ marginTop: "6.25rem", marginBottom: "3rem" }}>
                  <img
                    src={PartyEmprtyImg}
                    style={{ width: "17.95rem", height: "7.5rem" }}
                  />
                </div>
        ...
        <PartyLoading
          isLoading={isLoading}
          style={{
            background: `#ffca17 url(${partyLoading}) no-repeat top center`,
      ...
      <DangerWrapPopup openStatus={enrollPopupStatus}>
        <DangerPopup openStatus={enrollPopupStatus}>
          ...
          <div
            className="spoqaBold"
            style={{ fontSize: "0.875rem", lineHeight: "1.4375rem" }}
          >
            파티 참여하기
          </div>
          <div
...
const EnrollCompeletePopUp = ({
  openStatus,
  openChatLink,
  onClickCompleteClose,
}) => {
  return (
...
const PartyContent = ({ data }) => {
  ...
  const onClickDetailButton = () => {
    alert(
      "9월 6일에 모두 서비스가 종료될 예정이므로 파티 가입이 불가능합니다. 그동안 모두를 이용해주셔서 진심으로 감사드립니다. 🙇🏻‍♂️🙇🏻‍♀️"
    );
    return;
    ...
    <>
      ...
      >
        <div style={{ display: "flex" }}>
            ...
            {data.platformInfo.color && data.platformInfo.initial ? (
              <div
                ...
                style={{
                  ...
                  backgroundColor: data.platformInfo.color,
                }}
              >
                <div
                  style={{
              ...
              <img
                src={data.platformInfo.serverImgUrl}
                style={{
                  width: "2.3125rem",
                  height: "2.3125rem",
                  borderRadius: "0.3125rem",
          ...
          >
            ...
            >
              ...
              <div
                className="notoMedium"
                style={{ color: "#acacac", lineHeight: "1.4375rem" }}
              >
                {data.platformInfo.registerType === "SERVER"
                  ? data.platformInfo.serverCategory
                  : data.platformInfo.customCategory}
              </div>
            </div>
          </div>
        </div>
        <div style={{ display: "flex", marginTop: "0.3125rem" }}>
          {Array(data.partyInfo.personnel)
            .fill(1)
            .map((e, index) => {
              return (
                <img
                  key={index}
                  src={
                    index < data.partyInfo.currentUserCount
                      ? ActiveDuckIcon
                      : DeActiveDuckIcon
                  }
                  style={{
        ...
        <CompleteWrap isCompelte={data.roomStatus === "COMPELETE"}>
          <CompleteTextWrap className="spoqaBold">
            매칭 완료 🎉
          </CompleteTextWrap>
        </CompleteWrap>
      </ContentWrap>
        ...
        <ContentDetailWrap>
          <DetailRowWrap>
            <DetailItemWrap>
              ...
              <DetailItemFillContent>
                {data.platformInfo.registerType === "SERVER"
                  ? data.platformInfo.serverName
                  : data.platformInfo.customName}
              </DetailItemFillContent>
            </DetailItemWrap>
            <DetailItemWrap>
